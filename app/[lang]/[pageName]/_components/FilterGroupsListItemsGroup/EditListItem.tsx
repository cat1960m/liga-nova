...
export const EditListItem = ({ currentData, pageName, params }: Props) => {
  return (
    <div style={{ width: "100%" }}>
      {pageName === TICKETS_PAGE_NAME ? (
        <EditSubscriptionItem currentData={currentData} params={params} />
      ) : null}
      {pageName === TRAINERS_PAGE_NAME ? (
        <EditTrainerItem currentData={currentData} params={params} />
      ) : null}
    </div>
  );
};
